...
public class ItemAscendedBow extends ItemBow {

    public ItemAscendedBow() {
        setMaxDamage(1800);
    }
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft) {
        ...
        if (!worldIn.isRemote) {
            float[] yaws = new float[]{-12.0F, -4.0F, 0.0F, 4.0F, 12.0F};
            for (float yawOffset : yaws) {
                ...
                entityArrow.shoot(player, player.rotationPitch, player.rotationYaw + yawOffset, 0.0F, velocity * 3.25F, 0.4F);
                if (velocity == 1.0F) {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        tooltip.add(TextFormatting.LIGHT_PURPLE + "Fires a burning five-arrow volley.");
    }
